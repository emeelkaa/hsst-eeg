...
class TransformerEncoderBlock(nn.Sequential):
    def __init__(self, emb_size, num_heads, drop_p=0.5, forward_expansion=4, forward_drop_p=0.5):
        super().__init__(
...
class Conformer(nn.Module):
    def __init__(self, emb_size=40, depth=6, num_heads=4, num_channels= 16, num_classes=1):
        super().__init__()
        self.patch_embedding = PatchEmbedding(emb_size, num_channels)
        self.encoder = TransformerEncoder(depth, emb_size, num_heads)
        self.clshead = ClassificationHead(emb_size, num_classes)
